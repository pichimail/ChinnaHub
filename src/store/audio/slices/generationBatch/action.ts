...
export const createGenerationBatchSlice = (set: Setter, get: () => AudioStore, _api?: unknown) =>
...
export class GenerationBatchActionImpl {
  ...
  removeGeneration = async (generationId: string): Promise<void> => {
    ...
    // Audio batch has only 1 generation, so delete the batch directly
    if (activeGenerationTopicId) {
  ...
  useCheckGenerationStatus = (
    ...
    return useClientDataSWR<GetGenerationStatusResult>(
      ...
      {
        onError: (error) => console.error('Audio generation status check error:', error),
  ...
  useFetchGenerationBatches = (topicId?: string | null): SWRResponse<GenerationBatch[]> =>
    useClientDataSWR<GenerationBatch[]>(
      ...
      async ([, topicId]: [string, string]) => {
        return generationBatchService.getGenerationBatches(topicId, 'audio');
      },
      {
        onSuccess: (data) => {
          const nextMap = {
