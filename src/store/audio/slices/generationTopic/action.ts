...
const FETCH_GENERATION_TOPICS_KEY = 'fetchAudioGenerationTopics';

const n = setNamespace('audioGenerationTopic');

type Setter = StoreSetter<AudioStore>;

export const createGenerationTopicSlice = (set: Setter, get: () => AudioStore, _api?: unknown) =>
...
export class GenerationTopicActionImpl {
  ...
  internal_createGenerationTopic = async (): Promise<string> => {
    ...
    const topicId = await generationTopicService.createTopic('audio');
