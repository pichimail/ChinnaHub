...
import { App, Modal, Slider } from 'antd';
...
const AudioTile = memo<{ batch: GenerationBatch; generation: Generation; index: number }>(
  ({ batch, generation, index }) => {
    ...
    const download = useCallback(async () => {
      if (!audioUrl) return;
      await downloadFile(audioUrl, `${safeName(title)}.mp3`, false);
      message.success(t('generation.download'));
    }, [audioUrl, message, t, title]);

    const loadLyrics = useCallback(async () => {
      if (!audioUrl) return;
      try {
        await audioService.getTimestampedLyrics(generation.id);
        message.success(t('generation.lyricsReady'));
      } catch {
        message.error(t('generation.failed'));
      }
    }, [audioUrl, generation.id, message, t]);
    ...
    return (
      <Block
        className={`${styles.album} audio-album ${isLoading ? styles.loading : ''}`}
        variant="filled"
      >
        {artUrl ? (
          <img alt={title} className={styles.artwork} src={artUrl} />
        ) : null}
        {!artUrl && !isLoading ? (
        ...
        {isLoading ? (
          <Flexbox align="center" justify="center" height="100%" style={{ position: 'relative', zIndex: 2 }}>
        ...
        <div className={styles.controls}>
          <Text fontSize={12} style={{ minWidth: 36 }}>{Math.floor(time / 60)}:{String(Math.floor(time % 60)).padStart(2, '0')}</Text>
        ...
        <div className={styles.volume}>
          <Icon icon={volume > 0 ? Volume2 : VolumeX} size={14} />
          <Slider vertical max={1} min={0} step={0.01} tooltip={{ open: false }} value={volume} onChange={(value) => { setVolume(value); if (audioRef.current) audioRef.current.volume = value; }} />
        </div>

        <Modal centered footer={null} open={shareOpen} title="Share this track" onCancel={() => setShareOpen(false)}>
          <Flexbox gap={12}>
            <InputLike value={shareUrl} />
            <Flexbox horizontal gap={10} style={{ flexWrap: 'wrap' }}>
              <Button onClick={() => navigator.clipboard.writeText(shareUrl)}>Copy link</Button>
              <Button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(shareUrl)}`)}>WhatsApp</Button>
              <Button onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`)}>Facebook</Button>
              <Button onClick={() => window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}`)}>X</Button>
              <Button onClick={() => navigator.share?.({ title, url: shareUrl })}>Contacts</Button>
            </Flexbox>
          </Flexbox>
        </Modal>
  ...
  <Block gap={8} variant="borderless">
    ...
    <Grid maxItemWidth={360} rows={batch.generations.length}>
      {batch.generations.map((generation, index) => (
        <AudioTile batch={batch} generation={generation} index={index} key={generation.id} />
      ))}
    </Grid>
