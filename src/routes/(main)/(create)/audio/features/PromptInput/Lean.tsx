...
const styleOptions = [
  ...
  'Folk',
  'Ambient',
].map((v) => ({ label: v, value: v.toLowerCase().replaceAll(' ', '-') }));
const seeds = [
  [
    ...
    'cinematic pop, Telugu-English fusion',
    'muddy mix, flat hook, harsh highs',
    28,
    84,
    'Heartline',
  ],
  [
    ...
    'festival dance, Indian percussion',
    'slow intro, weak bass, dull drop',
    58,
    88,
    'Neon Jathara',
  ],
  [
    ...
    'lo-fi romance, soft piano',
    'busy drums, harsh tuning, noisy ambience',
    36,
    76,
    'Rain Notes',
  ],
...
const LeanPromptInput = memo<PromptInputProps>(({ showTitle = false }) => {
  ...
  useEffect(() => {
    if (!isInit) store.initializeAudioConfig();
  }, [isInit, store]);
  ...
  const setStyle = (v?: string) => {
    store.setAudioStyle(v?.slice(0, LIMIT.style));
    if (v) store.setAudioNegativeTags(negativeFromStyle(v));
  };
  const surprise = () => {
    const s = seeds[Math.floor(Math.random() * seeds.length)];
    store.setAudioPrompt(s[0].slice(0, max));
    store.setAudioStyle(s[1]);
    store.setAudioNegativeTags(s[2]);
    store.setAudioWeirdness(s[3]);
    store.setAudioStyleInfluence(s[4]);
    store.setAudioTitle(s[5]);
  };

  const generate = async () => {
    if (!isLogin) return loginRequired.redirect({ timeout: 2000 });
    const prompt = parameters.prompt?.trim() || '';
    if (mode === 'simple' && prompt.length > LIMIT.prompt)
      return message.warning('Simple prompt must stay under 500 characters.');
    if (mode === 'advanced' && prompt.length > LIMIT.lyrics)
      return message.warning('Lyrics description must stay under 5000 characters.');
    if ((parameters.style?.length || 0) > LIMIT.style)
      return message.warning('Music styles must stay under 1000 characters.');
    if ((parameters.negativeTags?.length || 0) > LIMIT.negative)
      return message.warning('Negative tags must stay under 200 characters.');
    ...
    const finalPrompt = hasContext
      ...
      : prompt;
    if (prompt) {
      store.appendAudioMessage({
        content: prompt,
        intent: hasContext ? 'followUp' : 'generate',
        role: 'user',
        trackContext: hasContext ? lastTrack : undefined,
      });
      store.appendAudioMessage({
        content: hasContext ? 'Creating the next version.' : 'Creating two Accoustica tracks.',
        intent: hasContext ? 'followUp' : 'generate',
        role: 'assistant',
        trackContext: hasContext ? lastTrack : undefined,
      });
  ...
  return (
    <Flexbox gap={32} width="100%">
      ...
      <GenerationPromptInput
        canGenerate={Boolean(parameters.prompt?.trim()) || refs.length > 0}
        disableGenerate={!isInit}
        generateLabel={t('generation.generate')}
        generatingLabel={t('generation.generating')}
        isCreating={isCreating}
        isDarkMode={isDarkMode}
        leftActions={<GenerationMediaModeSegment mode="audio" />}
        value={parameters.prompt || ''}
        inlineContent={
          <InlineImageReference
            images={refs}
            maxCount={1}
            onAdd={(d) => store.setAudioImageUrl(typeof d === 'string' ? d : d.url)}
            onRemove={() => store.setAudioImageUrl(undefined)}
          />
        }
        placeholder={
          mode === 'advanced' ? 'Write lyrics or describe the song structure' : 'Describe your song'
        }
        rightActions={
      ...
      >
        <Flexbox gap={14}>
          ...
          {mode === 'advanced' && (
            <Flexbox gap={6}>
              <Text weight={500}>Lyrics</Text>
              <Input.TextArea
                showCount
                autoSize={{ minRows: 5, maxRows: 10 }}
                maxLength={LIMIT.lyrics}
                placeholder="Write lyrics or describe the song structure"
                value={parameters.prompt || ''}
          ...
          <Input.TextArea
            ...
            maxLength={LIMIT.style}
            placeholder="Music styles"
            value={parameters.style || ''}
            onChange={(e) => setStyle(e.target.value)}
          />
          <Input.TextArea
            autoSize={{ minRows: 2, maxRows: 3 }}
            maxLength={LIMIT.negative}
            placeholder="Negative tags"
            value={parameters.negativeTags || ''}
            onChange={(e) => store.setAudioNegativeTags(e.target.value.slice(0, LIMIT.negative))}
          />
          <div>
            Weirdness: {parameters.weirdness || 1}
            <Slider
              max={100}
              min={1}
              value={parameters.weirdness || 1}
              onChange={store.setAudioWeirdness}
            />
          </div>
          <div>
            Style influence: {parameters.audioStyleInfluence || 1}
            <Slider
              max={100}
              min={1}
              value={parameters.audioStyleInfluence || 1}
              onChange={store.setAudioStyleInfluence}
            />
          </div>
          <Input
            placeholder="Title"
            value={parameters.title || ''}
            onChange={(e) => store.setAudioTitle(e.target.value)}
          />
          <Input
            placeholder="Artist"
            value={parameters.artist || ''}
            onChange={(e) => store.setAudioArtist(e.target.value)}
          />
          <Flexbox horizontal align="center" justify="space-between">
            <span>Instrumental</span>
            <Switch checked={!!parameters.makeInstrumental} onChange={store.setMakeInstrumental} />
          </Flexbox>
          <Flexbox horizontal gap={8}>
            <Select
              ...
              options={[
                { label: 'Accoustica', value: 'classic' },
                { label: 'Lyria', value: 'lyria' },
              ]}
              onChange={(v) => store.setAudioProviderMode(v as 'classic' | 'lyria')}
            />
            <Select
              style={{ width: 120 }}
              ...
              onChange={(v) => store.setAudioModelVersion(v as 'V1.0' | 'V2.0' | 'V3.0')}
            />
          </Flexbox>
          <Button block icon={<Icon icon={Dice5} />} onClick={surprise}>
            Surprise me
          </Button>
        </Flexbox>
      </Modal>
